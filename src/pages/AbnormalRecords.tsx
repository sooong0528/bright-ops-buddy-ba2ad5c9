...
import { abnormalRecords, type AbnormalRecord } from "@/lib/mockData";
...
export default function AbnormalRecords() {
  ...
  const recStats = useMemo(() => {
    ...
    // 已分析：已经进入或完成分析（含分析中、已恢复中经过分析的，简化为存在 analysisTaskId 且未忽略）
    const analyzed = abnormalRecords.filter((r) => !!r.analysisTaskId && r.status !== "已忽略").length;
    const recovered = abnormalRecords.filter((r) => r.status === "已恢复").length;
    const ignored = abnormalRecords.filter((r) => r.status === "已忽略").length;
    return { pending, analyzed, recovered, ignored };
  }, []);
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <SummaryTile icon={Bot} label="已分析" value={String(recStats.analyzed)} tone="info" />
      ...
      <div className="panel">
        ...
        <div className="flex items-center justify-between px-5 py-3 border-t bg-muted/20 text-xs text-muted-foreground">
          ...
          <span>待处理 {recStats.pending} · 已分析 {recStats.analyzed} · 已恢复 {recStats.recovered} · 已忽略 {recStats.ignored}</span>
        </div>
      </div>
...
// Silence unused import warning for AlertCircle/AlertTriangle if not used elsewhere.
void AlertCircle; void AlertTriangle;
